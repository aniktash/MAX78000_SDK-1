...
def read_input_from_binfile(file_name, index):
    out = Path(file_name).read_bytes()
    im_size = 88

    offset = index*im_size*im_size*16*3
    print(f'start from frame {index + 1} of {len(out) //(im_size*im_size*16*3)}')

    output = read_from_bin(out[offset:], 16, im_size)
    output = output.astype(np.int8)
    unfolded_inp = unfold_image(output, 4, 3)
    im_size = 352
    unfolded_inp = unfolded_inp.transpose(1,2,0)
    return unfolded_inp.astype(np.int8) + 128
...
def main():
    ...
    parser.add_argument("-i", "--input",
                        ...
                        help = "input binary or hex text file")
    ...
    parser.add_argument("-n", "--cnn_in",  action = 'store_true',
                        default = False, required = False,
                        help = "display the CNN input memory as an image")
    parser.add_argument("-s", "--save",  action = 'store_true',
                        default = False, required = False,
                        help = "save images with frame number")

    ar = parser.parse_args()
    print(ar.input)
    if ar.count is not None:
        if ar.cnn_in:
            folded_image = read_input_from_binfile(ar.input, ar.count) # read the CNN_IN from a file
            plt.imshow(folded_image)
            if ar.save:
                plt.savefig(f'cnn_in_saved_{Path(ar.input).stem}_{ar.count}.jpg')
            ...
            plt.imshow(colors)
            if ar.save:
                plt.savefig(f'cnn_out_saved_{Path(ar.input).stem}_{ar.count}.jpg')
            plt.show(block = False)
            # plt.pause(1)
            plt.pause(1)
    else:
        for i in range(99999): # large number, will break at the end
            try:
                if ar.cnn_in:
                    folded_image = read_input_from_binfile(ar.input, i) # read the CNN_IN from a file
                else:
            ...
            if ar.cnn_in:
                plt.imshow(folded_image)
                if ar.save:
                    plt.savefig(f'cnn_in_saved_{Path(ar.input).stem}_{i}.jpg')
            else:
                plt.imshow(colors)
                if ar.save:
                    plt.savefig(f'cnn_out_saved_{Path(ar.input).stem}_{i}.jpg')
            plt.title(str(i))
            plt.show(block = False)
            plt.pause(.3)
