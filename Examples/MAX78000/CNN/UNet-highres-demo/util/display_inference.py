...
def read_output_from_txtfile(file_name):
    ...
    outputhex = buf
    im_size = 88
    output = read_from_hex_list(outputhex, 16, im_size)
    output = output.astype(np.int8)
    unfolded_inp = unfold_image(output, 4, 4)
    im_size = 352
    colors = create_color_map(unfolded_inp, im_size)
...
def read_output_from_binfile(file_name):
    out = Path(file_name).read_bytes()

    im_size = 88
    output = read_from_bin(out, 16, im_size)
    output = output.astype(np.int8)
    unfolded_inp = unfold_image(output, 4, 4)
    im_size = 352
    colors = create_color_map(unfolded_inp, im_size)
...
def main():
    ...
    parser.add_argument("-i", "--input",
                        #required = False, default = "streaming-reference-cut-hex.txt",
                        required = False, default = "streaming-reference-cut.out",
    ...
    if ar.binary:
        colors = read_output_from_binfile(ar.input)
    else:
        colors = read_output_from_txtfile(ar.input)
    plt.imshow(colors)
